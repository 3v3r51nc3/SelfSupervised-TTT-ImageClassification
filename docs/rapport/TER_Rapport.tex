...
\newcommand{\supervisor}{
	Pr. Karine Ayoub, Pr. Camille Kurtz et Pr. Laurent Wendling
}
...
\begin{document}
	...
	\begin{titlepage}
		...
		% LOGO
		% -----------------------------------------------------
		\includegraphics[width=0.22\textwidth]{../presentation/assets/universite-paris-cite-logo.png}
		...
		\begin{flushleft}
			\large
			\textbf{Auteurs :} \authorname \\[0.4cm]
			
			\textbf{Encadrants :} \supervisor \\[0.4cm]
	...
	\begin{itemize}
		\item le Chapitre~2 présente l’état de l’art sur l’apprentissage auto-supervisé, le \emph{distribution shift} et le Test-Time Training ;
	...
	Les méthodes contrastives dominent le domaine depuis 2020.
	...
	La perte NT-Xent maximise la similarité des paires positives et minimise celle des paires négatives.
	...
	Contrairement aux CNN, un ViT ne repose pas sur des convolutions locales.
	...
	\begin{itemize}
		\item le modèle apprend la classification et la rotation ;
		\item la tête de rotation est conservée pour le test.
	\end{itemize}
	
	Au test :
	\begin{itemize}
		\item les labels ne sont pas disponibles ;
		\item un pas de gradient est effectué sur la perte de rotation ;
	...
	La tête de rotation est conservée pour l’adaptation TTT.
	...
	\begin{enumerate}
		...
		\item \textbf{Un projecteur MLP} $g(\cdot)$ à deux couches, qui mappe les représentations dans un espace latent de dimension 128.
		\item \textbf{La perte contrastive NT-Xent}, appliquée sur les paires positives $(z_i, z_j)$ issues des deux vues d’une même image.
	\end{enumerate}
	
	Cette architecture permet d’apprendre des représentations invariantes aux transformations visuelles, qui servent ensuite de base au fine-tuning supervisé et au Test-Time Training.
	...
	Le Test-Time Training (TTT) consiste à adapter le modèle au moment du test en exploitant une tâche auto-supervisée disponible même en l’absence de labels. 
	...
	\begin{itemize}
		...
		\item \textbf{adapt\_scope : ``encoder\_plus\_head''} — adaptation conjointe de l’encodeur ViT et de la tête de rotation ;
		\item \textbf{lambda\_rot : 1.0} — poids de la perte de rotation ;
	...
	Cependant, la tâche auxiliaire de rotation reste exploitable : pour chaque image de test, on peut générer une version tournée et calculer la perte de rotation correspondante.
	
	Le TTT suit donc les étapes suivantes :
	\begin{enumerate}
		\item générer une ou plusieurs rotations de l’image de test ;
		\item calculer la perte de rotation $\mathcal{L}_{\text{rot}}$ ;
	...
	Dans nos expériences, ce mode est celui qui fournit les meilleurs résultats, avec un gain moyen de +0.39\% sur CIFAR-10-C.
	...
	Nous utilisons :
	\begin{itemize}
		\item \textbf{encoder\_plus\_head} : adaptation conjointe du backbone ViT-Tiny et de la tête de rotation.
	\end{itemize}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lc}
			...
			\midrule
			Fine-tuning (val, meilleure époque) & \textbf{90.08\%} \\
			Baseline CIFAR-10 clean (sans TTT) & 89.59\% \\
			\bottomrule
		\end{tabular}
	...
	\begin{table}[H]
		...
		\begin{tabular}{r|ccc|cc}
			...
			\midrule
			1 & 86.73\% & 86.79\% & 86.66\% & +0.07\% & -0.06\% \\
			2 & 83.92\% & 84.09\% & 83.95\% & +0.17\% & +0.03\% \\
			3 & 80.33\% & 80.59\% & 80.33\% & 0.00\% & 0.00\% \\
			4 & 75.16\% & 75.75\% & 75.34\% & +0.59\% & +0.18\% \\
			5 & 68.03\% & 68.90\% & 68.30\% & +0.87\% & +0.27\% \\
			\midrule
			\textbf{moyenne} & \textbf{78.83\%} & \textbf{79.22\%} & \textbf{78.92\%} & \textbf{+0.39\%} & \textbf{+0.09\%} \\
			\bottomrule
		\end{tabular}
	...
	Le mode \textbf{per-image} est instable : il améliore légèrement les résultats aux sévérités faibles mais régresse sur le sous-ensemble apparié.
	...
	Le mode per-image dégrade les performances dans 46 cellules sur 70.
	
	\paragraph{Top gains per-batch.}
	
	Les plus fortes améliorations concernent les corruptions lourdes (fog, pixelate, noise), avec des gains jusqu’à \textbf{+2.19\%}.
	...
	\begin{table}[H]
		...
		\begin{tabular}{lc}
			...
			\midrule
			Baseline & 78.83\% \\
			SSL + Fine-tuning & 89.59\% (clean) \\
			SSL + Fine-tuning + TTT per-batch & \textbf{79.22\%} \\
			SSL + Fine-tuning + TTT per-image & 78.92\% \\
			\bottomrule
		\end{tabular}
	...
	\begin{itemize}
		\item un gain moyen de \textbf{+0.39\%} sur l’ensemble des corruptions ;
		...
		\item un gain croissant avec la sévérité, atteignant \textbf{+0.87\%} à sévérité 5.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item son gain moyen est négatif (\textbf{-0.48\%}) ;
	...
	\begin{enumerate}
		...
		\item \textbf{Le scope d’adaptation est large} : adapter l’encodeur complet et la tête de rotation peut provoquer une dérive rapide des représentations.
	\end{enumerate}
	...
	\begin{itemize}
		\item le TTT \textbf{per-batch} améliore systématiquement la robustesse du modèle, avec un gain moyen de +0.39\% et jusqu’à +0.87\% aux sévérités les plus élevées ;
		...
		\item l’ablation \texttt{steps=5} confirme l’hypothèse de sur-adaptation : le per-batch s’améliore fortement (+1.50\%), tandis que le per-image s’effondre (-1.68\%) ;
