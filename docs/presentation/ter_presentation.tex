...
\definecolor{parisciteBordeaux}{RGB}{154,20,70}
\definecolor{parisciteDark}{RGB}{35,35,35}
\definecolor{parisciteLight}{RGB}{247,245,246}
\definecolor{accent}{RGB}{154,20,70}
\newcommand{\pariscitelogo}{assets/universite-paris-cite-logo.png}

\setbeamercolor{normal text}{fg=parisciteDark,bg=white}
\setbeamercolor{title}{fg=parisciteBordeaux}
\setbeamercolor{subtitle}{fg=parisciteDark}
\setbeamercolor{author}{fg=parisciteDark}
\setbeamercolor{institute}{fg=parisciteDark}
\setbeamercolor{date}{fg=parisciteDark}
\setbeamercolor{frametitle}{fg=parisciteBordeaux,bg=white}
\setbeamercolor{structure}{fg=parisciteBordeaux}
\setbeamercolor{block title}{fg=white,bg=parisciteBordeaux}
\setbeamercolor{block body}{fg=parisciteDark,bg=parisciteLight}
\setbeamercolor{alerted text}{fg=parisciteBordeaux}
\setbeamercolor{author in head/foot}{fg=parisciteDark,bg=white}
\setbeamercolor{title in head/foot}{fg=parisciteDark,bg=white}
\setbeamercolor{date in head/foot}{fg=parisciteDark,bg=white}
\setbeamertemplate{blocks}[rounded][shadow=false]
\setbeamertemplate{itemize item}{\color{parisciteBordeaux}$\blacktriangleright$}
\setbeamertemplate{itemize subitem}{\color{parisciteBordeaux}$\triangleright$}
\setbeamertemplate{frametitle}{%
  \nointerlineskip%
  \begin{beamercolorbox}[wd=\textwidth,ht=0.76cm,dp=0.18cm,leftskip=0.1cm,rightskip=0.1cm]{frametitle}
    \usebeamerfont{frametitle}\insertframetitle\hfill%
    \raisebox{-0.08cm}{\includegraphics[height=0.46cm]{\pariscitelogo}}%
  \end{beamercolorbox}%
  {\color{parisciteBordeaux}\rule{\textwidth}{0.55pt}}%
}
\setbeamertemplate{footline}{%
  \leavevmode%
  \hbox{%
    \begin{beamercolorbox}[wd=.72\paperwidth,ht=2.5ex,dp=1ex,leftskip=.35cm]{title in head/foot}%
      \usebeamerfont{title in head/foot}\insertshorttitle%
    \end{beamercolorbox}%
    \begin{beamercolorbox}[wd=.18\paperwidth,ht=2.5ex,dp=1ex,center]{author in head/foot}%
      \usebeamerfont{author in head/foot}Master 1 VMI%
    \end{beamercolorbox}%
    \begin{beamercolorbox}[wd=.10\paperwidth,ht=2.5ex,dp=1ex,rightskip=.35cm plus1fil]{date in head/foot}%
      \usebeamerfont{date in head/foot}\insertframenumber/\inserttotalframenumber%
    \end{beamercolorbox}%
  }%
  \vskip0pt%
  {\color{parisciteBordeaux}\rule{\paperwidth}{0.4pt}}%
}
...
\subtitle{TER --- soutenance finale}
\author[Dolhov \& Diouf]{Maksym \textsc{Dolhov} \and Fallou \textsc{Diouf}}
\institute[U.~Paris]{Université Paris Cité --- Master 1 Vision et Machine Intelligente}
\date{28 mai 2026}
\titlegraphic{\includegraphics[height=1.0cm]{\pariscitelogo}}
...
\begin{frame}{Objectifs du TER}
  ...
  \vspace{0.5em}
  \begin{block}{Présentation finale}
    Nous présentons le pipeline réalisé, les résultats expérimentaux obtenus sur CIFAR-10-C et les enseignements tirés de l'étude du Test-Time Training auto-supervisé.
  \end{block}
\end{frame}
...
\begin{frame}{Stage C --- Test-Time Training (Sun et al. 2020)}
  ...
  \begin{itemize}
    ...
    \item Puis prédire avec la tête de classification, qui reste gelée.
  \end{itemize}
...
\begin{frame}{Schéma d'adaptation au test}
  \centering
  \begin{tikzpicture}[
      font=\scriptsize,
      node distance=0.52cm,
      box/.style={draw, rounded corners, align=center, minimum height=0.72cm, inner xsep=5pt, inner ysep=4pt},
      arr/.style={->, thick},
      upd/.style={->, thick, dashed, red!75!black}
    ]
    \node[box, fill=blue!8, minimum width=2.2cm] (x) {batch test\\$x$};
    \node[box, right=of x, fill=orange!15, minimum width=2.8cm] (rot) {copies tournées\\$0^\circ,90^\circ,180^\circ,270^\circ$};
    \node[box, right=of rot, fill=accent!18, text=white, minimum width=2.9cm] (rotmodel) {encodeur $f_{\theta_e}$\\+ tête rot $h_{\theta_r}$};
    \node[box, right=of rotmodel, fill=red!8, minimum width=2.3cm] (loss) {perte rotation\\$\mathcal L_{rot}$};
    \node[box, below=0.52cm of loss, fill=yellow!20, minimum width=3.8cm] (update) {1 pas gradient\\$(\theta_e,\theta_r)\to(\theta'_e,\theta'_r)$};

    \draw[arr] (x) -- (rot);
    \draw[arr] (rot) -- (rotmodel);
    \draw[arr] (rotmodel) -- (loss);
    \draw[upd] (loss) -- (update);
    \draw[upd] (update.west) to[out=180,in=-90] node[below, font=\tiny] {encodeur + tête rot adaptés} (rotmodel.south);

    \node[box, below=2.15cm of x, fill=blue!8, minimum width=2.2cm] (xpred) {même image\\$x$};
    \node[box, right=of xpred, fill=accent!18, text=white, minimum width=2.9cm] (enc) {encodeur adapté\\$f_{\theta'_e}$};
    \node[box, right=of enc, fill=green!15, minimum width=2.9cm] (cls) {tête classification\\gelée};
    \node[box, right=of cls, fill=green!8, minimum width=1.5cm] (y) {prédiction\\$\hat y$};

    \draw[arr] (xpred) -- (enc);
    \draw[arr] (enc) -- (cls);
    \draw[arr] (cls) -- (y);

    \node[above=0.08cm of rot, text=parisciteBordeaux, font=\bfseries\scriptsize] {Adaptation sans labels};
    \node[above=0.08cm of enc, text=parisciteBordeaux, font=\bfseries\scriptsize] {Prédiction après adaptation};
    \node[below=0.25cm of cls, text=red!70!black, font=\scriptsize] {Le label vrai n'est jamais utilisé pendant l'adaptation.};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Accuracy sur données propres (CIFAR-10 clean)}
  ...
  \begin{tabular}{lc}
    ...
    \midrule
    Stage B.2 fine-tune (val, époque 30, best) & \textbf{90,08\,\%} \\
    Stage C clean --- baseline (sans TTT)      & 89,59\,\% \\
    Stage C clean --- per-batch TTT            & 89,60\,\% \;($\Delta\!+\!0{,}01\,\%$) \\
    Stage C clean --- per-image TTT (sous-éch. 1k) & 89,80\,\% \\
    \bottomrule
  \end{tabular}
...
\begin{frame}{CIFAR-10-C --- accuracy moyenne par sévérité}
  ...
  \begin{tabular}{r|ccc|cc}
    ...
    \midrule
    1 & 86,73\,\% & 86,79\,\% & 86,66\,\% & \good{+0,07\,\%} & \bad{$-$0,06\,\%} \\
    2 & 83,92\,\% & 84,09\,\% & 83,95\,\% & \good{+0,17\,\%} & +0,03\,\% \\
    3 & 80,33\,\% & 80,59\,\% & 80,33\,\% & \good{+0,25\,\%} & 0,00\,\% \\
    4 & 75,16\,\% & 75,75\,\% & 75,34\,\% & \good{+0,59\,\%} & \good{+0,18\,\%} \\
    5 & 68,03\,\% & 68,90\,\% & 68,30\,\% & \good{+0,87\,\%} & \good{+0,27\,\%} \\
    \midrule
    \textbf{moyenne} & \textbf{78,83\,\%} & \textbf{79,22\,\%} & \textbf{78,92\,\%} & \good{\textbf{+0,39\,\%}} & \textbf{+0,09\,\%} \\
    \bottomrule
  \end{tabular}

  \vspace{0.6em}
  \begin{itemize}\footnotesize
    \item Per-batch : gain qui croît avec la sévérité (+0,07\,\% à sév.~1 $\to$ +0,87\,\% à sév.~5).
    \item *$\Delta$ image vs baseline 10k. Sur le sous-ensemble apparié 1k (apples-to-apples), per-image donne $\Delta$ moyen de \bad{$-0{,}48\,\%$}.
  \end{itemize}
\end{frame}
...
\begin{frame}{Top des gains per-batch (toutes sévérités confondues)}
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    fog            & 5 & 53,67\,\% & 55,86\,\% & \good{\textbf{+2,19\,\%}} \\
    pixelate       & 5 & 73,78\,\% & 75,30\,\% & \good{+1,52\,\%} \\
    impulse\_noise & 5 & 39,59\,\% & 41,09\,\% & \good{+1,50\,\%} \\
    pixelate       & 4 & 76,34\,\% & 77,72\,\% & \good{+1,38\,\%} \\
    impulse\_noise & 4 & 55,47\,\% & 56,75\,\% & \good{+1,28\,\%} \\
    gaussian\_noise & 4 & 55,56\,\% & 56,79\,\% & \good{+1,23\,\%} \\
    \bottomrule
  \end{tabular}
...
\begin{frame}{Ablations TTT --- effet de \code{steps} et \code{lr}}
  ...
  \begin{tabular}{lcc|cc}
    ...
    \midrule
    \textbf{Default} (\code{steps=1, lr=1e-3}) & \good{+0,39\,\%} & \good{+0,87\,\%} & \bad{$-$0,48\,\%} & \bad{$-$1,24\,\%} \\
    \textbf{steps=5} (\code{lr=1e-3})          & \good{\textbf{+1,50\,\%}} & \good{\textbf{+3,21\,\%}} & \bad{\textbf{$-$1,68\,\%}} & \bad{\textbf{$-$3,97\,\%}} \\
    \textbf{lr=1e-4} (\code{steps=1})          & +0,04\,\%        & +0,10\,\%        & $-$0,05\,\%       & $-$0,11\,\% \\
    \bottomrule
  \end{tabular}

  \vspace{0.6em}
  \begin{itemize}\footnotesize
    \item \alert{\code{steps=5} amplifie les deux effets en miroir} : per-batch passe à 67/70 wins (max +6,78\,\% à fog s5) ; per-image chute à 9/57 wins (min $-9{,}20\,\%$ à impulse\_noise s5).
...
\begin{frame}{Résumé quantitatif des runs}
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    Default            & \good{+0,39\,\%} & \bad{$-$0,48\,\%} & 61 / 6 / 3   & 16 / 46 / 8 \\
    steps=5            & \good{+1,50\,\%} & \bad{$-$1,68\,\%} & \good{67 / 2 / 1} & \bad{9 / 57 / 4} \\
    lr=1e-4            & +0,04\,\%        & $-$0,05\,\%       & 45 / 12 / 13 & 10 / 25 / 35 \\
    \bottomrule
  \end{tabular}
  ...
  \begin{block}{Lecture en une phrase}
    Seul \alert{per-batch avec adaptation \emph{plus agressive} (\code{steps=5})} produit des gains francs et fiables (+1,5\,\% moyen, +3,2\,\% à sévérité~5, 67/70 cellules améliorées). \\ Per-image dégrade systématiquement, et l'effet s'aggrave quand on adapte plus.
  \end{block}
\end{frame}

% =====================================================================
\section{Interprétation et perspectives}
...
\begin{frame}{Interprétation des résultats}
  \begin{enumerate}
    ...
    \item \textbf{L'ablation \code{steps=5} confirme l'hypothèse de sur-adaptation} : per-batch s'améliore encore (+1,5\,\% moyen, 67/70 wins), per-image s'effondre davantage ($-1{,}68\,\%$ moyen, jusqu'à $-9{,}2\,\%$). Adapter \emph{plus} aide en mode batch et casse en mode image.
...
\begin{frame}{Perspectives}
  \begin{itemize}
    ...
    \item \textbf{Rapport final} : intégrer les figures consolidées (par corruption, courbes severité) et les limites expérimentales.
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusion}
  \begin{itemize}
    ...
    \item Premier run overnight reproduit la signature attendue de TTT en mode \textbf{per-batch} (+0,4\,\% en moyenne, croissant avec la sévérité).
  ...
  \vspace{0.6em}
  \begin{block}{Bilan de la soutenance}
    \begin{enumerate}\small
      \item Présenter le travail réalisé de bout en bout pour le projet TER.
      \item Défendre les choix méthodologiques : SimCLR, ViT-Tiny et adaptation TTT par rotation.
      \item Résumer les résultats obtenus, leurs limites et les perspectives ouvertes.
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}[plain]
  \begin{center}
    ...
    {\footnotesize TER --- Université Paris Cité --- Master 1 Vision et Machine Intelligente --- Mai 2026}
  \end{center}
\end{frame}
...
% --- Backup : architecture du dépôt ----------------------------------
\begin{frame}{Annexe --- Architecture du dépôt}
  \centering
  \small
  \begin{tabular}{ll}
    \toprule
...
\begin{frame}{Annexe B --- Seed unique, comment quantifier le bruit ?}
  \qbox{\guillemotleft\,Comment savez-vous que le +0,4\,\% de per-batch n'est pas du bruit de seed ?\,\guillemotright{}}
  ...
  \begin{itemize}
    ...
    \begin{itemize}\small
      \item monotonie sur la sévérité ($+0{,}07 \to +0{,}87$\,\%), peu probable par chance ;
    ...
    \item Pour la per-image dégradation ($\Delta = -0{,}48$\,\% sur 1k), le signe est cohérent à travers toutes les sévérités $\geq$ 1 ; même argument de monotonie tient.
  \end{itemize}
\end{frame}
...
\begin{frame}{Annexe C --- ViT/LN vs ResNet/BN : choix défendable ?}
  ...
  \begin{itemize}
    ...
    \item Notre +0,4\,\% est donc \alert{purement par adaptation des poids via la perte rotation}, pas via une renormalisation BN.
...
\begin{frame}{Annexe G --- Coût computationnel de TTT}
  \qbox{\guillemotleft\,Vous gagnez 0,4\,\% avec per-batch et 4--8\,\% à sévérité 5. Combien ça coûte en temps d'inférence ?\,\guillemotright{}}
  ...
  \begin{tabular}{lcc}
    ...
    per-batch TTT      & +0,4\,\% ($\sim$+0,9\,\% à sév.~5) & $\sim$2$\times$ (1 fwd + 1 bwd) \\
    per-image TTT      & $-0{,}5$\,\% & $\sim$130$\times$ (128 copies + bwd) \\
    \bottomrule
  \end{tabular}
...
\begin{frame}{Annexe L --- Pourquoi notre question sur la tête rotation conjointe ?}
  ...
  \begin{itemize}\small
    ...
    \item Notre gain per-batch modeste en default ($+0{,}39$\,\%) pourrait s'expliquer en partie par un encodeur \emph{tiraillé} entre les deux objectifs.
  \end{itemize}
